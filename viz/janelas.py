...
from viz.campo_anim import (  # animacao de esforco no campo (compartilhado)
    _build_period_maps,
    _tabela_e_anima_esforcos,
)
                            ...
                            if sensor_points:
                                ...
                                with st.spinner("Calculando janelas temporais..."):
                                    ...
                                    if _tj:
                                        ...
                                        # ── Esforços do jogador plotados no campo ──
                                        if dados_posicao_por_periodo and _vj:
                                            _mx_ind = float(np.nanmax(_vj))
                                            if _mx_ind > 0:
                                                _alta_ind, _media_ind = (
                                                    encontrar_eventos_nao_sobrepostos(
                                                        _tj, _vj, window_minutes,
                                                        round(_mx_ind * 0.90, 1),
                                                        round(_mx_ind * 0.75, 1),
                                                        _mx_ind))
                                                _maps_ind = _build_period_maps(
                                                    dados_sensor_por_atleta_por_periodo,
                                                    _jan_modo_todos, periodo_janela,
                                                    dados_posicao_por_periodo)
                                                _tabela_e_anima_esforcos(
                                                    sorted(_alta_ind + _media_ind,
                                                           key=lambda e: e['valor'],
                                                           reverse=True),
                                                    [atleta_janela], _maps_ind,
                                                    dados_posicao_por_periodo,
                                                    window_minutes, _unidade_jan,
                                                    tipo_metrica, key="ev_ind_sel",
                                                    titulo=("#### 🎬 Esforços de "
                                                            f"{atleta_janela} no Campo"),
                                                    offset_min=_maps_ind['atl_offset'](
                                                        atleta_janela),
                                                    min_atl=1)
                                    else:
                            ...
                            if not _pos_sel:
                                ...
                                with st.spinner("Calculando por posição..."):
                                    ...
                                    if not _atl_res:
                                        ...
                                        _pos_summ: list = []
                                        _pos_curves: dict = {}   # posição → curva média (no grid _t_grid_pos)
                                        ...
                                        # ── Esforços da posição plotados no campo ──
                                        if dados_posicao_por_periodo and _pos_curves:
                                            st.markdown("---")
                                            st.markdown(
                                                "#### 🎬 Esforços da Posição no Campo")
                                            _pa_sel = st.selectbox(
                                                "Posição para ver no campo:",
                                                list(_pos_curves.keys()),
                                                key="jan_pos_anim_sel")
                                            _vm_pos = _pos_curves[_pa_sel]
                                            _mx_pos = (float(np.nanmax(_vm_pos))
                                                       if len(_vm_pos) else 0.0)
                                            if _mx_pos > 0:
                                                _vm_ev = np.where(
                                                    np.isfinite(_vm_pos),
                                                    _vm_pos, -1.0).tolist()
                                                _alta_pos, _media_pos = (
                                                    encontrar_eventos_nao_sobrepostos(
                                                        _t_grid_pos.tolist(), _vm_ev,
                                                        window_minutes,
                                                        round(_mx_pos * 0.90, 1),
                                                        round(_mx_pos * 0.75, 1),
                                                        _mx_pos))
                                                _maps_pos = _build_period_maps(
                                                    dados_sensor_por_atleta_por_periodo,
                                                    _jan_modo_todos, periodo_janela,
                                                    dados_posicao_por_periodo)
                                                _players_pos = [
                                                    _a for _a in _pos_atls.get(_pa_sel, [])
                                                    if _a in _atl_res]
                                                _tabela_e_anima_esforcos(
                                                    sorted(_alta_pos + _media_pos,
                                                           key=lambda e: e['valor'],
                                                           reverse=True),
                                                    _players_pos, _maps_pos,
                                                    dados_posicao_por_periodo,
                                                    window_minutes, _unidade_jan,
                                                    tipo_metrica, key="ev_pos_sel",
...
<div style="background:linear-gradient(135deg,rgba(25,35,55,0.65) 0%,rgba(15,25,45,0.45) 100%);
  ...
  <strong style="color:white">{_tmx:.1f} {_unidade_jan}</strong>.
                    ...
                    # ── RHIE — Esforços Repetidos de Alta Intensidade ──────────────
                    render_rhie_secao(
                        dados_sensor_por_atleta_por_periodo,
                        dados_posicao_por_periodo, _SENSOR_HZ, "janelas")
